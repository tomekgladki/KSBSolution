...
def edit_preferences(request):
    ...
    if request.method == 'POST':
        ...
        selected_countries = request.POST.getlist('countries')
        selected_sectors = request.POST.getlist('sectors')
        selected_criteria = request.POST.get('function_criteria')  # Zmienione pole na `function_criteria`
        selected_weights = request.POST.get('weight_criteria')  # Zmienione pole na `weight_criteria`

        # Sprawdzenie, czy użytkownik zaznaczył kryterium
        if not selected_criteria:
            messages.error(request, "You must select at least one criterion.")
            return redirect('edit_preferences')

        # Sprawdzenie, czy użytkownik zaznaczył metodę wag
        if not selected_weights:
            messages.error(request, "You must select a weighting method.")
            return redirect('edit_preferences')
...
def wrap_wallet(request):
    if request.method == "POST":
        try:
            ...
            criteria = request.POST.get("function_criteria")  # Kryterium wyboru spółek
            weight = request.POST.get("weight_criteria")  # Metoda wyznaczania wag
            ...
            print(f"Countries: {countries}")
            print(f"Sectors: {sectors}")
            print(f"Criteria: {criteria} | Weight: {weight}")

            # Uruchomienie funkcji wrapper z przekazaniem parametrów
            prices_df, weights = wrapper(
                countries=countries, 
                sectors=sectors, 
                criteria=criteria, 
                weight=weight,
                ...
                beta_1=float(beta_1),
                beta_2=float(beta_2),
                sharpe_1=float(sharpe_1),
                sharpe_2=float(sharpe_2),
                a=float(a),
